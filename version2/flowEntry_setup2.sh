#add group entry
ovs-ofctl -O openflow15 add-group s1 group_id=1,type=select,selection_method=hash,bucket=output:1,bucket=output:2,bucket=output:3
ovs-ofctl -O openflow15 add-group s2 group_id=1,type=select,selection_method=hash,bucket=output:1,bucket=output:2,bucket=output:3
...
ovs-ofctl -O openflow15 add-flow s2 in_port=2,dl_type=0x0800,nw_src=172.18.0.1,nw_dst=172.18.0.2,priority=10,idle_timeout=0,hard_timeout=0,actions=output:4
...
ovs-ofctl -O openflow15 add-flow s1 in_port=1,dl_type=0x0800,nw_src=172.18.0.2,nw_dst=172.18.0.1,priority=10,idle_timeout=0,hard_timeout=0,actions=output:4
