...
//for non equal probability path selection
//int p = 2;
//int q = 1;
//int base = 0;
//mod end
...
xlate_hash_fields_select_group(struct xlate_ctx *ctx, struct group_dpif *group)
{
    ...
    if(!strcasecmp("s1", datapath_name)){
         if(fp1 == NULL){
            if((fp1 = fopen("/home/tank/flowlet/version2/flowlet_log_s1.txt", "a+")) == NULL) {
    ...
    if(!strcasecmp("s2", datapath_name)){
         if(fp2 == NULL){
            if((fp2 = fopen("/home/tank/flowlet/version2/flowlet_log_s2.txt", "a+")) == NULL) {
                printf("open log file failed!");
                exit(0);
            }
        }
